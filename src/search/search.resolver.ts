...
import { SearchContentsArgs, SearchContentsFilterType, SearchContentsPayload } from "./dto/search-contents.dto";
    ...
    if (type === null) {
      return this.service.searchAll(query, { skip, limit }).pipe(
        map((val) => ({
          results: val.results
            .filter(
              (value): value is { id: string; type: ContentType; } => value !== null,
            )
            .map(({ id, type }) => ({
              content: { id, type },
            })),
